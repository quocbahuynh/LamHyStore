...
interface GomOrderCampaign {
  id: string;
  name: string;
  slug: string;
  startDate: string; // Hoặc Date nếu muốn dùng kiểu Date
  endDate: string; // Hoặc Date nếu muốn dùng kiểu Date
  productExternalIds: string[];
}

interface SubMenuComponent {
  data: GomOrderCampaign;
}

interface GomOrderUpdate {
  name: string;
  startDate: string;
  endDate: string;
...
const GomOrderItem = ({ data }: SubMenuComponent) => {
  ...
  const handleConfirmDelete = async () => {
    await axios.delete(`${apiLinks.order.deleteById}/${data.id}`);
  ...
  const handleUpdate = async () => {
    const payload = {
      ...
      startDate: getValues().startDate.trim(),
      endDate: getValues().endDate.trim(),
      addProductExternalIds: addedProductIds,
      removeProductExternalIds: removedProductIds,
    };

    await axios.patch(`${apiLinks.order.updateById}/${data.id}`, payload, {
      headers: {
  ...
  const handleCopy = () => {
    const textToCopy = `${apiLinks.domain.domain}/g/${data.slug}`;
    navigator.clipboard
  ...
  return (
    <tr>
      <td>{data.name}</td>
      <td>
        <Link href={apiLinks.domain.domain + `/g/${data.slug}`}>
          {apiLinks.domain.domain + `/g/${data.slug}`}
        </Link>
      </td>
        ...
        <Modal.Body>
          <div className="row" style={{ height: "60vh" }}>
            <div className="col-6">
              ...
              <Form.Group>
                <InputGroup className="mb-3 mt-4 gap-4 justify-content-between">
                  <Form.Group>
                    <Form.Label>Ngày bắt đầu (Tháng - Ngày - Năm)</Form.Label>
                    <Form.Control
                      type="date"
                      placeholder="Tiêu đề"
                  ...
                  <Form.Group>
                    <Form.Label>Ngày kết thúc (Tháng - Ngày - Năm)</Form.Label>
                    <Form.Control
                      type="date"
                      placeholder="Tiêu đề"
...
export default function page() {
  ...
  const [searchQuery, setSearchQuery] = useState("");
  const [productsSearch, setProductsSearch] = useState([]);
  const { register, handleSubmit, reset } = useForm<AddGomOrder>();
  const handleSearch = async () => {
    setSearching(true);
    if (!searchQuery.trim()) {
      alert("Vui lòng nhập tên sản phẩm.");
  ...
  const onSubmit = async (payload: AddGomOrder) => {
    try {
      ...
      await axios.post(apiLinks.order.createGomOrder, payload);
      reset();
      location.reload();
    } catch (error) {
      console.error(error);
  ...
  const [gomOrder, setGomOrder] = useState<GomOrderCampaign[]>([]);

  useEffect(() => {
    const fetchGomOrders = async () => {
      try {
        const response = await fetch(apiLinks.order.gomOrderGetAll); // Thay API URL của bạn
        if (!response.ok) {
          throw new Error("Failed to fetch data");
        }
        const data: GomOrderCampaign[] = await response.json();
        setGomOrder(data);
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Tạo Gom Order</Modal.Title>
        </Modal.Header>
