  ...
  const onSubmit = async (payload: SectionUpdatePayload) => {
    try {
      if (data) {
        ...
        await axios.put(
          apiLinks.admin.sectionBestSellerUpdated + "/" + data.id,
          payload,
        );
        reset();
        location.reload();
...
export default function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(apiLinks.homepage.bestseller);
  ...
  const onSubmit = async (payload: SectionUpdatePayload) => {
    try {
      ...
      payload.productExternalIds = extractProductsId;
      await axios.post(apiLinks.admin.sectionBestSellerAdd, payload);
      reset();
      location.reload();
    } catch (error) {
      console.error(error);
      ...
      <div className="dashboard-page-content">
        <div className="row mb-9 align-items-center justify-content-between">
          <div className="col-md-6 mb-8 mb-md-0">
            <h2 className="fs-4 mb-0">Top sản phẩm bán chạy</h2>
          </div>
